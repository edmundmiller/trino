...
Declare a [user-defined function](/docs/udf). 
...
storage](/docs/admin/properties-sql-environment).

The list of parameters is a comma-separated list of names `parameter_name` and
data types `data_type`, see [data type](/docs/language/types). An empty list, specified as
`()` is also valid.

The `type` value after the `RETURNS` keyword identifies the [data
type](/docs/language/types) of the UDF output.
...
any non-deterministic UDFs and [functions](/docs/functions). By default, UDFs are
...
documentation](/docs/udf/sql/examples) and the [Python UDF
documentation](/docs/udf/python).
