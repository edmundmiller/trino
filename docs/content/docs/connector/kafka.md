...
See the [kafka-tutorial](/docs/kafka-tutorial).
...
When using Protobuf decoder with the [Confluent table description
supplier<confluent-table-description-supplier>](#Confluent table description
...
- Copy the `kafka-protobuf-provider` and `kafka-protobuf-types` JAR files
  ...
  The plugin directory depends on the [/installation](/docs//installation) method.
...
In addition, you must configure [table schema and schema registry usage
<kafka-table-schema-registry>](#table schema and schema registry usage
...
additional steps are necessary. For details, refer to [kafka-requirements](#kafka-requirements).
...
specific types are supported in addition to the [normally supported types
<kafka-protobuf-decoding>](#normally supported types
...
The Kafka connector supports the use of [/sql/insert](/docs//sql/insert) statements to write
...
connector [maps some types <type-mapping-overview>](#maps some types <type-mapping-overview>) when reading
([decoding <kafka-row-decoding>](#decoding <kafka-row-decoding>)) or writing ([encoding
<kafka-row-encoding>](#encoding
...
tables populated by Kafka topics. See [kafka-row-decoding](#kafka-row-decoding) for more
information.

In addition to the [globally available <sql-globally-available>](#globally available <sql-globally-available>)
and [read operation <sql-read-operations>](#read operation <sql-read-operations>) statements, the connector
supports the following features:

- [/sql/insert](/docs//sql/insert), encoded to a specified data format. See also
  [kafka-sql-inserts](#kafka-sql-inserts).
