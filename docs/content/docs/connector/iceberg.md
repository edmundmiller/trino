---
title: Iceberg
description: Iceberg documentation
---
# Iceberg connector
...
individual data files.  Trino queries using the [/connector/hive](/docs//connector/hive) must
...
- Access to a [Hive metastore service (HMS) <hive-thrift-metastore>](#Hive metastore service (HMS) <hive-thrift-metastore>), an
  [AWS Glue catalog <iceberg-glue-catalog>](#AWS Glue catalog <iceberg-glue-catalog>), a [JDBC catalog
  <iceberg-jdbc-catalog>](#JDBC catalog
  <iceberg-jdbc-catalog>), a [REST catalog <iceberg-rest-catalog>](#REST catalog <iceberg-rest-catalog>),
  a [Nessie server <iceberg-nessie-catalog>](#Nessie server <iceberg-nessie-catalog>), or a
  [Snowflake catalog <iceberg-snowflake-catalog>](#Snowflake catalog <iceberg-snowflake-catalog>).
...
with [general metastore configuration properties
<general-metastore-properties>](#general metastore configuration properties
...
|---|---|---|
| `iceberg.catalog.type` | Define the [metastore type](general-metastore-properties) to use. Possible values are: * `hive_metastore` * `glue` * `jdbc` * `rest` * `nessie` * `snowflake` | `hive_metastore` |
| `iceberg.file-format` | Define the data storage file format for Iceberg tables. Possible values are: * `PARQUET` * `ORC` * `AVRO` | `PARQUET` |
| `iceberg.compression-codec` | The compression codec used when writing files. Possible values are: * `NONE` * `SNAPPY` * `LZ4` * `ZSTD` * `GZIP` | `ZSTD` |
...
| `iceberg.table-statistics-enabled` | Enable [](/optimizer/statistics). The equivalent [catalog session property](/docs/sql/set-session) is `statistics_enabled` for session specific use. Set to `false` to prevent statistics usage by the [](/optimizer/cost-based-optimizations) to make better decisions about the query plan and therefore improve query processing performance. Setting to `false` is not recommended and does not disable statistics gathering. | `true` |
| `iceberg.extended-statistics.enabled` | Enable statistics collection with [](/sql/analyze) and use of extended statistics. The equivalent catalog session property is `extended_statistics_enabled`. | `true` |
...
| `iceberg.projection-pushdown-enabled` | Enable [projection pushdown](/docs/optimizer/pushdown) | `true` |
| `iceberg.hive-catalog-name` | Catalog to redirect to when a Hive table is referenced. |  |
| `iceberg.register-table-procedure.enabled` | Enable to allow user to call [`register_table` procedure](iceberg-register-table). | `false` |
| `iceberg.add-files-procedure.enabled` | Enable to allow user to call [`add_files` procedure](iceberg-add-files). | `false` |
...
| `iceberg.expire-snapshots.min-retention` | Minimal retention period for the [`expire_snapshot` command](iceberg-expire-snapshots). Equivalent session property is `expire_snapshots_min_retention`. | `7d` |
| `iceberg.remove-orphan-files.min-retention` | Minimal retention period for the  [`remove_orphan_files` command](iceberg-remove-orphan-files). Equivalent session property is `remove_orphan_files_min_retention`. | `7d` |
...
Iceberg. In addition to the [globally available <sql-globally-available>](#globally available <sql-globally-available>)
and [read operation <sql-read-operations>](#read operation <sql-read-operations>) statements, the connector
supports the following features:

- [sql-write-operations](#sql-write-operations):

  - [iceberg-schema-table-management](#iceberg-schema-table-management) and [iceberg-tables](#iceberg-tables)
  - [iceberg-data-management](#iceberg-data-management)
  - [sql-view-management](#sql-view-management)
  - [sql-materialized-view-management](#sql-materialized-view-management), see also [iceberg-materialized-views](#iceberg-materialized-views)
...
`location` set in [/sql/create-table](/docs//sql/create-table) statement, are located in a
...
The Iceberg connector supports creating tables using the [CREATE TABLE
](/docs//sql/create-table) syntax. Optionally, specify the [table properties
...
The Iceberg connector supports creating tables using the [CREATE TABLE AS
](/docs//sql/create-table-as) with [SELECT ](/docs//sql/select) syntax:
...
Another flavor of creating tables with [CREATE TABLE AS
](/docs//sql/create-table-as) is with [VALUES ](/docs//sql/values) syntax:
...
Given the table definition from [Partitioned Tables <iceberg-tables>](#Partitioned Tables <iceberg-tables>)
...
- [/sql/alter-table](/docs//sql/alter-table)
...
[ALTER TABLE SET PROPERTIES <alter-table-set-properties>](#ALTER TABLE SET PROPERTIES <alter-table-set-properties>).
...
The current values of a table's properties can be shown using [SHOW CREATE
TABLE ](/docs//sql/show-create-table).
...
| Name | Type | Description |
|---|---|---|
| `status` | `INTEGER` | Numeric status indication to track additions and deletions. Deletes are informational only and not used in scans: * `EXISTING(0)` * `ADDED(1)` * `DELETED(2)` |
...
[/sql/drop-table](/docs//sql/drop-table) syntax. When the command succeeds, both the data of the
...
the [/sql/create-table](/docs//sql/create-table) operation.

The `COMMENT` option is supported for adding table columns through the
[/sql/alter-table](/docs//sql/alter-table) operations.

The connector supports the command [COMMENT ](/docs//sql/comment) for setting
...
using the [CREATE TABLE ](/docs//sql/create-table) syntax:
...
The Iceberg connector supports [sql-materialized-view-management](#sql-materialized-view-management). In the
...
You can use the [iceberg-table-properties](#iceberg-table-properties) to control the created storage
table and therefore the layout and performance. For example, you can use the
following clause with [/sql/create-materialized-view](/docs//sql/create-materialized-view) to use the ORC format
...
must run [/sql/refresh-materialized-view](/docs//sql/refresh-materialized-view) to populate data in the
materialized view.
...
Dropping a materialized view with [/sql/drop-materialized-view](/docs//sql/drop-materialized-view) removes
...
The Iceberg connector can collect column statistics using [/sql/analyze](/docs//sql/analyze)
...
[/sql/analyze](/docs//sql/analyze) on tables may improve query performance by collecting
...
dropped using the [drop_extended_stats <drop-extended-stats>](#drop_extended_stats <drop-extended-stats>) command
before re-analyzing.
...
implementation](/docs/object-storage/file-system-cache). If `fs.cache.enabled` is
...
#### Memory metadata caching configuration properties

| Property | Description |
|---|---|
| `fs.memory-cache.ttl` | The maximum [duration](prop-type-duration) to keep files in the cache prior to eviction. The minimum value of `0s` means that caching is effectively turned off. Defaults to `1h`. |
| `fs.memory-cache.max-size` | The maximum total [data size](prop-type-data-size) of the cache. When raising this value, keep in mind that the coordinator memory is used. Defaults to `200MB`. |
