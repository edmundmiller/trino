---
title: Release 468
description: Release 468 documentation
---
# Release 468 (17 Dec 2024)

## General

* Add support for [](/udf/python). ([#24378](https://github.com/trinodb/trino/issues/24378))
* Add cluster overview to the [](/admin/preview-web-interface). ([#23600](https://github.com/trinodb/trino/issues/23600))
...
* Improve performance when reading external
  [BigLake](https://cloud.google.com/bigquery/docs/biglake-intro) tables. ([#21016](https://github.com/trinodb/trino/issues/21016))
...
* {{breaking}} Reduce coordinator memory usage for the Delta table metadata
  ...
  `delta.metadata.cache-ttl` to `30m`. ([#24432](https://github.com/trinodb/trino/issues/24432))
...
* Enable mismatched bucket execution optimization by default. This can be
  ...
  and the `optimize_mismatched_bucket_count` session property. ([#23432](https://github.com/trinodb/trino/issues/23432))
* Improve performance by deactivating bucket execution when not useful in query
  processing. ([#23432](https://github.com/trinodb/trino/issues/23432))
...
* Improve performance when running a join or aggregation on a bucketed table
  ...
  `bucket_execution_enabled` session property. ([#23432](https://github.com/trinodb/trino/issues/23432))
* Deprecate the `iceberg.materialized-views.storage-schema` configuration
  property. ([#24398](https://github.com/trinodb/trino/issues/24398))  
* {{breaking}} Rename the `partitions` column in the `$manifests` metadata table
  to `partition_summaries`. ([#24103](https://github.com/trinodb/trino/issues/24103))
* Avoid excessive resource usage on coordinator when reading Iceberg system
  tables. ([#24396](https://github.com/trinodb/trino/issues/24396))

## PostgreSQL connector

* Add support for non-transactional [MERGE statements](/docs/sql/merge). ([#23034](https://github.com/trinodb/trino/issues/23034))

## SPI

* Add partitioning push down, which a connector can use to activate optional
  partitioning or choose between multiple partitioning strategies. ([#23432](https://github.com/trinodb/trino/issues/23432))
